...
import {
  ...
  Clock,
  TrendingUp,
} from "lucide-react"; // THÊM Clock, TrendingUp
...
export default function Header() {
  ...
  // ✅ Dùng hook useLogout: clearCart + logout + removeQueries(['cart', ...]) + gỡ header
  ...
  const renderEmptySearchContent = () => (
    <div className="p-4 space-y-4">
      ...
      <div>
        <div className="flex items-center justify-between text-xs font-medium text-gray-500 mb-2">
          <span className="flex items-center gap-1">
            <Clock className="w-4 h-4" /> Lịch sử tìm kiếm (Mock)
          </span>
      ...
      <div>
        <p className="flex items-center gap-1 text-xs font-medium text-gray-500 mb-2">
          <TrendingUp className="w-4 h-4" /> Xu hướng tìm kiếm (Mock)
        </p>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs font-semibold text-blue-600">
                  {formatPrice(
                    Number(p.variations?.[0]?.price || p.base_price) *
                      (1 - Number(p.discount_percentage || 0) / 100)
                  )}
                </div>
              </div>
            </button>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isMenuOpen && (
          <div className="md:hidden py-4 border-t border-gray-200">
            ...
            <nav className="flex flex-col gap-4">{/* ... Menu Mobile */}</nav>
          </div>
        )}
      </div>
    </header>
