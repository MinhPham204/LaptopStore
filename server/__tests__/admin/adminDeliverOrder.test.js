...
const deliverUrl = (id = ORDER_ID) => `/api/admin/orders/${id}/deliver`
...
const postDeliver = (id = ORDER_ID, token = signSessionToken(ADMIN_USER_ID)) =>
  request(app).post(deliverUrl(id)).set("Authorization", `Bearer ${token}`).send({})

describe("POST /api/admin/orders/:order_id/deliver (deliverOrder)", () => {
  let order
  ...
  it("returns 200 and updates order to delivered for admin when status is shipping", async () => {
    const res = await postDeliver()

    expect(res.status).toBe(200)
    expect(res.body.message).toBe("Order delivered successfully")
    expect(order.update).toHaveBeenCalledWith({ status: "delivered" })
    expect(order.status).toBe("delivered")
  })

  it("returns 200 for manager when status is shipping", async () => {
    const res = await postDeliver(ORDER_ID, signSessionToken(MANAGER_USER_ID))
  ...
  it("returns 400 when order status is processing", async () => {
    order.status = "processing"

    const res = await postDeliver()

    expect(res.status).toBe(400)
    expect(res.body.message).toBe("Order must be in shipping status to deliver")
    expect(order.update).not.toHaveBeenCalled()
  })

  it.each([["delivered"], ["cancelled"], ["AWAITING_PAYMENT"]])(
    "returns 400 when order status is %s",
    async (status) => {
    order.status = status
  ...
  it("returns 401 without bearer token", async () => {
    const res = await request(app).post(deliverUrl()).send({})
  ...
  it("returns 403 for customer role", async () => {
    const res = await postDeliver(ORDER_ID, signSessionToken(CUSTOMER_USER_ID))
  ...
  it("returns 403 for staff role", async () => {
    const res = await postDeliver(ORDER_ID, signSessionToken(STAFF_USER_ID))
